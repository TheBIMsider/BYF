  ...
  updateQuickStats() {
    ...
    if (weightToGoEl && this.currentUser) {
      const weightToGo = Math.abs(this.currentUser.currentWeight - this.currentUser.goalWeight);
  ...
  loadTodaysData() {
    ...
    if (weightInput && todaysLog.weight !== null && todaysLog.weight !== undefined) {
    ...
    document.querySelectorAll('.exercise-checkbox').forEach((cb) => (cb.checked = false));
    document.querySelectorAll('.wellness-checkbox').forEach((cb) => (cb.checked = false));
  ...
  getAppStats() {
    ...
    const weightLogs = Object.values(this.dailyLogs).filter((log) => log.weight !== null).length;
    const totalExerciseMinutes = Object.values(this.dailyLogs).reduce((sum, log) => sum + (log.exerciseMinutes || 0), 0);
    const totalSteps = Object.values(this.dailyLogs).reduce((sum, log) => sum + (log.steps || 0), 0);
    const totalWater = Object.values(this.dailyLogs).reduce((sum, log) => sum + (log.water || 0), 0);
    ...
    const stats = {
      ...
      longestStreak: Math.max(this.streaks.steps, this.streaks.exercise, this.streaks.water, this.streaks.wellness, this.streaks.overall),
    ...
    if (this.currentUser) {
      stats.weightProgress = {
        ...
        remaining: Math.abs(this.currentUser.currentWeight - this.currentUser.goalWeight),
      };
    }
  ...
  // Include all chart rendering methods from the original version
  // These remain identical as they handle visualization logic
  ...
  renderWeightChart() {
    ...
    const xScale = (index) => padding.left + (index / (weightData.length - 1)) * chartWidth;
    const yScale = (weight) => padding.top + (1 - (weight - minWeight) / (maxWeight - minWeight)) * chartHeight;
    ...
    // Draw goal line
    if (this.currentUser.goalWeight >= minWeight && this.currentUser.goalWeight <= maxWeight) {
      const goalY = yScale(this.currentUser.goalWeight);
      ctx.strokeStyle = getComputedStyle(document.documentElement).getPropertyValue('--accent-success');
      ctx.lineWidth = 2;
      ctx.setLineDash([5, 5]);
      ctx.beginPath();
      ...
      ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--accent-success');
      ctx.font = 'bold 12px sans-serif';
      ctx.textAlign = 'left';
      ctx.fillText(`Goal: ${this.currentUser.goalWeight} lbs`, width - padding.right - 100, goalY - 10);
    }
  ...
  renderActivityChart() {
    ...
    const maxSteps = Math.max(...activityData.map((d) => d.steps), this.currentUser.dailySteps);
    const maxExercise = Math.max(...activityData.map((d) => d.exercise), this.currentUser.dailyExercise);
    const maxWater = Math.max(...activityData.map((d) => d.water), this.currentUser.dailyWater);
    ...
    activityData.forEach((data, index) => {
      ...
      ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--accent-primary');
      ctx.fillRect(x, padding.top + chartHeight - stepsHeight, barWidth / 3, stepsHeight);

      const exerciseHeight = (data.exercise / maxExercise) * chartHeight;
      ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--accent-success');
      ctx.fillRect(x + barWidth / 3, padding.top + chartHeight - exerciseHeight, barWidth / 3, exerciseHeight);

      const waterHeight = (data.water / maxWater) * chartHeight;
      ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--accent-warning');
      ctx.fillRect(x + (barWidth * 2) / 3, padding.top + chartHeight - waterHeight, barWidth / 3, waterHeight);
    ...
    if (periodEl) {
      periodEl.textContent = this.chartPeriod === 'all' ? 'All Time' : `Last ${this.chartPeriod} Days`;
    }
  }
  ...
  navigateCalendar(direction) {
    ...
    this.renderCalendarMonth(this.currentCalendarMonth, this.currentCalendarYear);
  }
  ...
  isMilestoneClaimedInAchievements(milestone) {
    return this.achievements.some((achievement) => achievement.type === milestone.type && achievement.value === milestone.value);
  }
  ...
  getCustomRewardForMilestone(milestone) {
    return this.customRewards.find((reward) => reward.type === milestone.type && (reward.streakDays === milestone.value || reward.weightLoss === milestone.value));
  }
  ...
  renderAchievementHistory() {
    ...
    const sortedAchievements = [...this.achievements].sort((a, b) => new Date(b.claimedDate) - new Date(a.claimedDate));
  ...
  checkAchievements() {
    ...
    this.defaultMilestones.filter((m) => m.type === 'streak').forEach((milestone) => {
      if (this.isMilestoneAchieved(milestone) && !this.isMilestoneClaimedInAchievements(milestone)) {
        newAchievements.push(milestone);
      }
    });

    this.defaultMilestones.filter((m) => m.type === 'weight').forEach((milestone) => {
      if (this.isMilestoneAchieved(milestone) && !this.isMilestoneClaimedInAchievements(milestone)) {
        newAchievements.push(milestone);
      }
    });
  ...
  showAchievementModal(achievement) {
    ...
    const rewardText = achievement.customReward ? ` Enjoy your reward: ${achievement.customReward.description}` : ` Consider setting a custom reward for future milestones!`;
...
window.addEventListener('beforeunload', (e) => {
  if (window.app && window.app.syncQueue && window.app.syncQueue.length > 0) {
    const unsyncedItems = window.app.syncQueue.filter(item => !item.synced).length;
    if (unsyncedItems > 0) {
      e.preventDefault();
...
window.addEventListener('offline', () => {
  ...
  if (window.app) {
    window.app.updateSyncStatus('offline', 'Offline - will sync when reconnected');
  }
});
...
window.addEventListener('appinstalled', () => {
  ...
  if (window.app) {
    ...
    window.app.showSuccess('🎉 App installed! You can now use BribeYourselfFit offline.');
  }
});
...
class BribeYourselfFitCloud {
  ...
  saveCloudConfig() {
    try {
      localStorage.setItem('byf_cloud_config', JSON.stringify(this.cloudConfig));
    } catch (error) {
   ...
   */
  async saveToCloud() {
    if (!this.cloudConfig.isConnected || this.cloudConfig.syncInProgress) return;
  ...
  /**
   * Handle sync errors with progressive fallback
   */
  handleSyncError(error) {
    ...
    if (this.cloudConfig.retryCount <= this.cloudConfig.maxRetries) {
      // Schedule retry
      const retryDelay = Math.min(1000 * Math.pow(2, this.cloudConfig.retryCount), 30000);
      setTimeout(() => {
      ...
      this.updateSyncStatus('error', `Retrying in ${Math.round(retryDelay/1000)}s...`);
    } else {
      // Max retries reached, fall back to local storage
      this.updateSyncStatus('offline', 'Offline mode - will sync when reconnected');
      if (this.syncNotifications) {
        this.showError('Sync failed. Data saved locally, will retry when connection is restored.');
      }
    }
  }
  ...
  startAutoSync() {
    ...
    this.autoSyncInterval = setInterval(() => {
      if (this.autoSyncEnabled && this.cloudConfig.isConnected && this.syncQueue.length > 0) {
        this.saveToCloud();
      }
    }, 5 * 60 * 1000);
  }
  ...
  updateSyncStatus(status = null, message = null) {
    ...
    if (dashboardSyncEl) {
      ...
      if (dotEl && labelEl) {
        ...
        labelEl.textContent = message || status.charAt(0).toUpperCase() + status.slice(1);
      }
    }
  }
  ...
  setupEventListeners() {
    ...
    if (cloudSetupForm) {
      ...
      if (toggleApiKeyBtn) {
        toggleApiKeyBtn.addEventListener('click', () => this.togglePasswordVisibility('jsonbinApiKey'));
      }
    }
    ...
    exerciseCheckboxes.forEach((checkbox) => {
      checkbox.addEventListener('change', this.updateExerciseSelection.bind(this));
    });
    ...
    if (rewardType) {
      rewardType.addEventListener('change', this.updateRewardCriteria.bind(this));
    }
  ...
  setupSettingsEventListeners() {
    ...
    if (updateApiKeyBtn) {
      updateApiKeyBtn.addEventListener('click', this.handleUpdateApiKey.bind(this));
    }
    ...
    if (toggleSettingsApiKey) {
      toggleSettingsApiKey.addEventListener('click', () => this.togglePasswordVisibility('settingsApiKey'));
    }
    ...
    if (syncNotifications) {
      syncNotifications.addEventListener('change', (e) => {
        ...
        localStorage.setItem('byf_sync_notifications', this.syncNotifications.toString());
      });
    }
  ...
  async handleTestConnection() {
    ...
    if (!apiKey) {
      this.updateConnectionStatus('error', 'Please enter your JSONBin.io API key');
      return;
    }
    ...
    if (success) {
      this.updateConnectionStatus('success', '✅ Connection successful! You can now complete your profile setup.');
      this.saveCloudConfig();
  ...
  async handleSetup(e) {
    ...
    // Validate input
    if (!this.validateSetupData(startingWeight, goalWeight, dailySteps, dailyExercise, dailyWater)) {
      return;
    }
    ...
    if (cloudSyncSuccess) {
      this.showSuccess('🎉 Profile created and synced to cloud! Start logging your fitness journey.');
    } else {
      this.showSuccess('Profile created! Data saved locally and will sync when cloud connection is restored.');
    }
  }
  ...
  async handleDailyLog(e) {
    ...
    const steps = stepsInput && stepsInput.value !== '' ? parseInt(stepsInput.value) : 0;
    const exerciseMinutes = exerciseInput && exerciseInput.value !== '' ? parseInt(exerciseInput.value) : 0;
    const water = waterInput && waterInput.value !== '' ? parseFloat(waterInput.value) : 0;

    // Get selected exercise types
    const exerciseTypes = Array.from(document.querySelectorAll('.exercise-checkbox:checked')).map((cb) => cb.value);

    // Get wellness score
    const wellnessItems = Array.from(document.querySelectorAll('.wellness-checkbox:checked')).map((cb) => cb.dataset.wellness);
    const wellnessScore = wellnessItems.length;

    // Validate data
    if (!(await this.validateDailyLog(weight, steps, exerciseMinutes, water, exerciseTypes))) {
      return;
    }
  ...
  loadLocalData() {
    try {
      ...
      this.streaks = JSON.parse(localStorage.getItem('byf_streaks')) || this.initializeStreaks();
      this.customRewards = JSON.parse(localStorage.getItem('byf_customRewards')) || [];
      this.achievements = JSON.parse(localStorage.getItem('byf_achievements')) || [];

      // Load preferences
      this.autoSyncEnabled = localStorage.getItem('byf_auto_sync') !== 'false';
      this.syncNotifications = localStorage.getItem('byf_sync_notifications') !== 'false';
  ...
  saveLocalData() {
    try {
      ...
      localStorage.setItem('byf_customRewards', JSON.stringify(this.customRewards));
      localStorage.setItem('byf_achievements', JSON.stringify(this.achievements));
    } catch (error) {
  ...
  downloadData(data, type) {
    ...
    link.href = url;
    link.download = `BribeYourselfFit_${type}_backup_${new Date().toISOString().split('T')[0]}.json`;
   ...
   */
  resetLocalData() {
    if (!confirm('This will delete all local data. Your cloud data will remain safe. Continue?')) {
      return;
    }

    // Clear localStorage
    const keysToRemove = ['byf_user', 'byf_dailyLogs', 'byf_streaks', 'byf_customRewards', 'byf_achievements'];
  ...
  async resetCloudData() {
    ...
    if (!confirm('This will permanently delete all data from cloud storage. This cannot be undone. Continue?')) {
      return;
    }

    if (!confirm('Are you absolutely sure? This will delete ALL your fitness data from the cloud!')) {
      return;
    }

    try {
      if (this.cloudConfig.binId) {
        const response = await fetch(`https://api.jsonbin.io/v3/b/${this.cloudConfig.binId}`, {
          method: 'DELETE',
          headers: {
            'X-Master-Key': this.cloudConfig.apiKey
          }
        ...
        if (response.ok) {
          ...
          this.saveCloudConfig();
          this.showSuccess('Cloud data deleted successfully. Local data preserved.');
        } else {
   ...
   */
  async resetAllData() {
    if (!confirm('This will delete ALL data from both local storage and cloud. This cannot be undone. Continue?')) {
      return;
    }

    if (!confirm('Final warning: This will permanently delete EVERYTHING. Are you sure?')) {
      return;
    }
    ...
    try {
      ...
      const keysToRemove = [
        'byf_user', 'byf_dailyLogs', 'byf_streaks', 'byf_customRewards', 
        'byf_achievements', 'byf_cloud_config', 'byf_auto_sync', 'byf_sync_notifications'
      ];
      keysToRemove.forEach(key => localStorage.removeItem(key));

      this.showSuccess('All data has been reset. Page will reload in 3 seconds...');
  ...
  updateSettingsDisplay() {
    ...
    if (connectionStatusEl) {
      connectionStatusEl.textContent = this.cloudConfig.isConnected ? '✅ Connected' : '❌ Disconnected';
    }
    ...
    if (totalEntriesEl) {
      totalEntriesEl.textContent = Object.keys(this.dailyLogs).length.toString();
    }
    ...
    const syncNotificationsCheckbox = document.getElementById('syncNotifications');
  ...
  // Include all the original methods from the localStorage version
  // (validateSetupData, validateDailyLog, updateStreaks, etc.)
  // These remain exactly the same as they handle the core fitness logic

  /**
   * Validate setup form data
   */
  validateSetupData(startingWeight, goalWeight, dailySteps, dailyExercise, dailyWater) {
    if (isNaN(startingWeight) || isNaN(goalWeight) || isNaN(dailySteps) || isNaN(dailyExercise) || isNaN(dailyWater)) {
  ...
  async validateDailyLog(weight, steps, exerciseMinutes, water, exerciseTypes) {
    // Validate weight
    if (weight && (weight < this.validationRanges.weight.min || weight > this.validationRanges.weight.max)) {
      if (!(await this.confirmUnusualValue('weight', weight, 'lbs'))) return false;
    }

    // Validate steps
    if (steps > this.validationRanges.steps.max) {
      if (!(await this.confirmUnusualValue('steps', steps, 'steps'))) return false;
    }

    // Validate exercise minutes
    if (exerciseMinutes > this.validationRanges.exercise.max) {
      if (!(await this.confirmUnusualValue('exercise', exerciseMinutes, 'minutes'))) return false;
    }

    // Validate water
    if (water > this.validationRanges.water.max) {
      if (!(await this.confirmUnusualValue('water', water, 'liters'))) return false;
    }

    // Validate exercise types are selected if exercise minutes > 0
    if (exerciseMinutes > 0 && exerciseTypes.length === 0) {
      this.showError('Please select at least one exercise type when logging exercise minutes.');
      return false;
    }
